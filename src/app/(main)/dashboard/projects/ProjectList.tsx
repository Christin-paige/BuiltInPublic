...
export default function ProjectList() {
  ...
  return (
    <div className='bg-gradient-to-b from-[#1d1d1d] to-[#86059F] rounded-md shadow p-3'>
      ...
      {projects.length === 0 ? (
        <p className='text-center text-sm text-slate-300 py-8'>
          No projects yet.
        </p>
      ) : (
        <div className='grid grid-cols-1 gap-3'>
          {projects.map((p) => (
            <ProjectCard
              key={p.id}
              href={`/projects/${p.id}`} // adjust if your route differs
              {...p} // ProjectCard expects inline fields (name, description, status, etc.)
            />
          ))}
        </div>
      )}
