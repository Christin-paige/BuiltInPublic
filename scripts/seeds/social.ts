...
export async function seedSocial() {
  ...
  for (const comment of comments) {
    const [{ data: commenterProfile }, { data: targetPosts }] =
      await Promise.all([
        supabase
          .from("profiles")
          .select("id")
          .eq("username", comment.commenter)
          .maybeSingle(),
        supabase
          .from("posts")
          .select("id, user_id")
          .eq(
            ...
            (
              await supabase
                .from("profiles")
                .select("id")
                .eq("username", comment.postAuthor)
                .maybeSingle()
            ).data?.id || "",
          )
    ...
    if (!commenterProfile || !postId) {
      console.warn(
        `Skipping comment from ${comment.commenter} on ${comment.postAuthor}`,
      );
      continue;
    }
    ...
    const [{ data: followerProfile }, { data: followingProfile }] =
      await Promise.all([
        ...
        supabase
          .from("profiles")
          .select("id")
          .eq("username", follow.following)
          .maybeSingle(),
      ]);

    if (!followerProfile || !followingProfile) {
      console.warn(
        `Skipping follow from ${follow.follower} to ${follow.following}`,
      );
      continue;
    }
  ...
  for (const endorsement of endorsements) {
    const [{ data: endorser }, { data: endorsed }, { data: skill }] =
      await Promise.all([
        supabase
          .from("profiles")
          .select("id")
          .eq("username", endorsement.user_id)
          .maybeSingle(),
        supabase
          .from("profiles")
          .select("id")
          .eq("username", endorsement.endorsed_to)
          .maybeSingle(),
        supabase
          .from("skills")
          .select("id")
          .eq("id", endorsement.skill_id)
          .maybeSingle(),
      ]);
    ...
    if (!endorser || !endorsed || !skill) {
      console.warn(
        `Skipping endorsement: ${endorsement.user_id} → ${endorsement.endorsed_to} for ${endorsement.skill_id}`,
      );
      continue;
    }
